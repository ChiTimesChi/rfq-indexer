...
// src/server/graphql/metadata.ts
import { GraphQLObjectType as GraphQLObjectType2 } from "graphql";
import { GraphQLJSON as GraphQLJSON2 } from "graphql-type-json";
var metadataEntity = new GraphQLObjectType2({
  name: "_meta",
  fields: { status: { type: GraphQLJSON2 } }
});
...
async function createServer2({
  ...
  }).get("/graphql", (c) => c.html(prodGraphiql)).post("/graphql", async (c) => {
    const status = await metadataStore.getStatus();
    if (status === null || Object.values(status).some(({ ready }) => ready === false)) {
      return c.json(
  ...
  }).get("/", (c) => c.html(rootGraphiql)).post("/", (c) => rootYoga.handle(c.req.raw)).get("/status", async (c) => {
    const status = await metadataStore.getStatus();
